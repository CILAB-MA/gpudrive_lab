...
def train(exp_config=None, master_seed=None, ddp_local_rank=None):
    ...
    bc_policy = DDP(bc_policy, device_ids=[ddp_local_rank])
    optimizer = AdamW(bc_policy.parameters(), lr=exp_config.lr, eps=0.0001)
    load_path = exp_config.get('load_path', None)
    if load_path and os.path.exists(load_path) and master_process:
    ...
    stop_training = False
    if not os.path.exists(model_path) and master_process:
    ...
    while gradient_steps < exp_config.total_gradient_steps and not stop_training:
        ...
        for n, batch in enumerate(expert_data_loader):
            ...
            # Evaluation loop
            if (gradient_steps % exp_config.eval_freq == 0) and master_process:
                bc_policy.eval()
...
if __name__ == "__main__":
    ...
    if master_process and args.use_wandb:
        ...
        if args.sweep_id is not None:
            wandb.agent(args.sweep_id, function=partial(train, master_process=master_process,ddp_local_rank=ddp_local_rank), project=private_info['main_project'], entity=private_info['entity'])
        else:
            sweep_id = wandb.sweep(sweep_config, project=private_info['main_project'], entity=private_info['entity'])
            wandb.agent(sweep_id, function=partial(train, master_process=master_process,ddp_local_rank=ddp_local_rank))
    else:
        train(exp_config)
