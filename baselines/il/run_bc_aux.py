...
def evaluate(eval_expert_data_loader, config, bc_policy, num_train_sample):
    ...
    for i, batch in enumerate(eval_expert_data_loader):
        ...
        if total_samples > num_train_sample / 10:
            ...
            break
        if len(batch) == 9:
            obs, expert_action, masks, ego_masks, partner_masks, road_masks, other_pos, aux_mask, _ = batch  
        elif len(batch) == 7:
        ...
        other_pos = other_pos.to(config.device)
        ...
        with torch.no_grad():
            ...
            if config.use_tom:
                ...
                tom_loss = LOSS['aux'](bc_policy, other_input, other_pos, aux_mask, 
                    aux_info=aux_info)
...
def train():
    ...
    expert_data_loader = get_dataloader(config.data_path, config.train_data_file, config)
    ...
    while gradient_steps < config.total_gradient_steps and not stop_training:
        ...
        for _, batch in enumerate(expert_data_loader):
            if gradient_steps >= config.total_gradient_steps:
                break
            if len(batch) == 9:
                obs, expert_action, masks, ego_masks, partner_masks, road_masks, other_pos, aux_mask, data_idx = batch
            elif len(batch) == 7:
            ...
            if config.use_wandb and (gradient_steps % config.log_freq == 0):
                log_dict = {   
                        ...
                        "train/tom_loss": tom_losses / config.log_freq ,
            ...
            if gradient_steps % config.eval_freq == 0:
                bc_policy.eval()
                test_loss, dx_loss, dy_loss, dyaw_loss, tom_loss = evaluate(eval_expert_data_loader, config, bc_policy, num_train_sample)
                if config.use_wandb:
                    with torch.no_grad():
