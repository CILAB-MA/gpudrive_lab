...
def run(args):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        for time_step in tqdm(range(env.episode_len)):
            ...
            with torch.no_grad():
                ...
                for (ego_name, ego_head) , (other_name, other_head) in zip(ego_lp_heads.items(), other_lp_heads.items()):
                    ...
                    ego_world = torch.zeros((args.batch_size, 1)).long().to("cuda")
                    ego_world_prime = torch.zeros((args.batch_size, 1)).long().to("cuda")
                    other_world_pred = torch.zeros((args.batch_size, 127)).long().to("cuda")
            ...
            if args.make_image:
                partner_idx = torch.full((args.batch_size,  127), -2).float().to('cuda')
                ...
                for future_step, other_lp in other_lp_dict.items():
                    ...
                    other_lp_dict[future_step] = other_lp[torch.arange(args.batch_size), intervention_match].unsqueeze(-1)
        ...
        if args.make_image:
            ...
            for world_render_idx in range(args.dataset_size):
                world_image_path = os.path.join(image_path, f'scene_{world_render_idx + idx * args.batch_size}')
                ...
                for time_step, frame in enumerate(frames[world_render_idx]):
                    filename = f'scene_{world_render_idx + idx * args.batch_size}_step_{time_step}.jpg'
