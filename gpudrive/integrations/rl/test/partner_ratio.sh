...
BATCH_SIZE=${1:-100}  # 기본값 200
DATASET_SIZE=${2:-5000}  # 기본값 5000
SWEEP_NAME=${3:-"scene_1000"}  # 기본값 "early_attn5000"
GPU_ID=${4:-0}  # 기본값 "early_attn5000"
PARTNER_AGENT=${5:-"self_play"}
# pp 값 리스트
PP_VALUES=(0.0 0.2 0.4 0.6 0.8 0.999) # max = 0.99

# 반복문 실행
for PP in "${PP_VALUES[@]}"; do
    python gpudrive/integrations/rl/test/run_simulation.py --sweep-name "$SWEEP_NAME" \
