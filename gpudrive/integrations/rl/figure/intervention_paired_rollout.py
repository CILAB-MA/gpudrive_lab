...
2. Branch baseline vs. intervention (LP probe weights into RL partner_embed tokens).
...
from gpudrive.networks.late_fusion import NeuralNet, sample_logits
...
def load_config(config_path: str = "baselines/ppo/config/ppo_base_puffer.yaml"):
    with open(config_path, "r") as f:
        config = Box(yaml.safe_load(f))
    return pufferlib.namespace(**config)


def load_rl_policy(model_path: str, config_path: str = "baselines/ppo/config/ppo_base_puffer.yaml"):
    """Load PPO NeuralNet from a ``*.pt`` checkpoint (``parameters`` state dict)."""
    config = load_config(config_path)
    params = torch.load(model_path, weights_only=False)
    sd = params["parameters"] if isinstance(params, dict) and "parameters" in params else params
...
def _partner_tokens(policy, obs):
    """Return partner_embed tokens (B, 127, D) before max-pool / LP embed edit."""
    _ego, road_objects, _rg = policy.unpack_obs(obs)
    return policy.partner_embed(road_objects).clone()


def _fusion_partner_tokens(policy, obs, masks=None):
    """Alias for IL-compatible call sites (masks unused for RL late-fusion)."""
    return _partner_tokens(policy, obs)
...
def get_context_with_partner_delta(policy, obs, masks=None, partner_delta=None):
    """Encode obs with optional partner-token delta (RL late-fusion).

    Mirrors ``intervention.py``: edit ``partner_embed`` tokens, max-pool, then
    ``shared_embed``. Returns the shared hidden used by actor/critic.
    """
    ego_state, road_objects, road_graph = policy.unpack_obs(obs)
    ego_embed = policy.ego_embed(ego_state)
    partner_tok = policy.partner_embed(road_objects)
    if partner_delta is not None:
        partner_tok = partner_tok + partner_delta
    partner_embed, _ = partner_tok.max(dim=1)
    road_map_embed, _ = policy.road_map_embed(road_graph).max(dim=1)
    embed = torch.cat([ego_embed, partner_embed, road_map_embed], dim=1)
    return policy.shared_embed(embed)
...
@torch.no_grad()
def fusion_partner_tokens(policy, obs, masks=None):
    """Return partner_embed tokens (B, 127, D) before the embed edit."""
    return _partner_tokens(policy, obs)
...
def step_policy_actions(
    ...
    if residual_spec is not None:
        tokens = _fusion_partner_tokens(policy, alive_obs)
        ...
        if world_apply_mask is not None:
            apply = world_apply_mask[world_ids].to(dtype=delta.dtype).view(-1, 1, 1)
            delta = delta * apply
    hidden = get_context_with_partner_delta(policy, alive_obs, partner_delta=delta)
    logits = policy.actor(hidden)
    actions, *_ = sample_logits(logits, action=None, deterministic=True)
...
def rollout_batch(
    ...
    for t in range(ep_len):
        ...
        if in_policy.any():
            ...
            # Expert prefix is continuous (W,A,3); RL policy is discrete (W,A).
            # Map indices → classic (accel, steer, head) via action_keys_tensor.
            if pol_actions.dim() == 2 and actions.dim() == 3:
                pol_cont = env.action_keys_tensor[pol_actions.long()]
                for w in torch.where(in_policy)[0].tolist():
                    actions[w] = pol_cont[w]
            else:
                for w in torch.where(in_policy)[0].tolist():
...
def parse_args():
    ...
    p.add_argument("--csv-path", "-cp", type=str, default="/data/after_cvpr/intervention.csv")
    p.add_argument(
        "--others-csv",
        type=str,
        default="/data/after_cvpr/intervention_others.csv",
    )
    p.add_argument(
        "--tgt-debug-csv",
        type=str,
    ...
    p.add_argument("--model-path", "-mp", type=str, default="/data/after_cvpr/rl/scene_10000")
    p.add_argument("--model-name", "-mn", type=str, default="", help="Checkpoint stem under model-path (auto-detect if empty)")
    p.add_argument("--lp-model-name", type=str, default="pos_lp")
...
def main():
    ...
    lp_root = os.path.join(args.model_path, "other_linear_prob", model_stem)
    other_lps = []
