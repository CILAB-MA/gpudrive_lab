...
# linear_probing
from gpudrive.integrations.rl.run_lp import register_all_layers_forward_hook
...
def run(args, env, policy, ego_lp_models, other_lp_models, intervention_idx, 
    ...
    first_alive_per_world = alive_agent_mask.long().argmax(dim=1)
    batch_idx = torch.arange(alive_agent_mask.shape[0], device=alive_agent_mask.device)
    ...
    policy_actions = torch.zeros((args.batch_size, 128, env.episode_len)).to("cuda").long()
    # ============================================================
    ego_idx = first_alive_per_world
    ...
    for time_step in tqdm(range(env.episode_len)):
        ...
        wm = ~dead_agent_mask[batch_idx, first_alive_per_world]
        ego_global_pos[:, time_step][wm] = torch.stack((ego_global_state.pos_x, ego_global_state.pos_y), dim=-1)[batch_idx, first_alive_per_world]
        ego_global_rot[:, time_step][wm] = ego_global_state.rotation_angle[batch_idx, first_alive_per_world]
        partner_pos = env.get_partner_pos()
        other_relative_pos[:, time_step][wm] = partner_pos[batch_idx, first_alive_per_world]
        other_relative_mask[:, time_step][wm] = lp_partner_mask_bool[batch_idx, first_alive_per_world]
        with torch.no_grad():
            # for padding zero
            alive_obs = obs[~dead_agent_mask]
            actions, *_ = policy(alive_obs, deterministic=True)
        policy_actions[..., time_step][~dead_agent_mask] = actions
    ...
    for time_step in tqdm(range(env.episode_len)):
        ...
        # MASK
        partner_mask = env.get_partner_mask().to("cuda")
        world_mask = ~dead_agent_mask.gather(
                1, first_alive_per_world.unsqueeze(1)
            ).squeeze(1)
        alpha = 5
        with torch.no_grad():
            # for padding zero
            alive_obs = obs[batch_idx, first_alive_per_world]
            with torch.no_grad():
                # for padding zero
                actions, *_ = policy(alive_obs, deterministic=True)
            if time_step < env.episode_len - 40 and time_step % 3 == 0: 
                ...
                for i, (other_lp, ego_lp, future_step) in enumerate(zip(other_lp_models, ego_lp_models, future_steps)):
                    ...
                    other_pred = other_lp(other_lp_input[wm])
                    ...
                    ego_orig_pred = ego_lp(ego_lp_input[wm])
                    ego_prime_pred = ego_lp(ego_input_prime[wm]) # todo: intervention idx applying
        ...
        # Align controlled_agent_mask with first_alive_per_world so LP plots render
        lp_controlled_mask = torch.zeros(NUM_WORLD, env.vis.controlled_agent_mask.shape[1], dtype=torch.bool, device=env.vis.device)
        idx = first_alive_per_world[:NUM_WORLD].to(env.vis.device)
        lp_controlled_mask[torch.arange(NUM_WORLD, device=env.vis.device), idx] = True
        setattr(env.vis, "ego_agent_mask", lp_controlled_mask)
