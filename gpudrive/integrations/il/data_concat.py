...
def run(save_path, save_name, subset_path, num_scenes, start_idx):
    ...
    ego_labels = []
    partner_labels = []
    for idx, (subset_data, global_data) in tqdm(enumerate(zip(subset_datas, global_datas,))): #label_datas))):
    ...
    np.savez_compressed(os.path.join(save_path + '/global', 'global_' + save_name), ego_global_rot=ego_global_rots,ego_global_pos=ego_global_poss )
    print("done!!!")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--num-scene', type=int, default=20000)
    parser.add_argument('--start-idx', type=int, default=10000)
    parser.add_argument('--dataset', type=str, default='training', choices=['training', 'validation', 'testing'],)
    args = parser.parse_args()
    save_path = "/data/full_version/processed/2000_subset/"
