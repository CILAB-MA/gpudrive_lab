"""Auto-label intervention scenes (type=i/r/n) from probing + expert rollouts.
...
def collect_batch_probe_features(
    ...
    """Expert-prefix + policy context hooks for every valid world in the batch.
    ...
    fusion_hooks = register_all_layers_forward_hook(policy.fusion_attn)
    ro_hooks = register_all_layers_forward_hook(policy.ro_attn)
    ...
    # Pass 2: reset and run again with get_context hooks for LP readout.
    obs = env.reset()
    dead = ~env.cont_agent_mask.clone()
    expert_actions, *_ = env.get_expert_actions()
    ...
    for t in range(ep_len):
        ...
        if (t % 3 == 0) and (t + 40 < ep_len):
            ...
            if live_ws:
                ...
                masks = [
                    partner_mask_bool[alive_w].unsqueeze(1),
                    road_mask[alive_w].unsqueeze(1),
                ]
                _ = policy.get_context(obs[alive_w], masks)
                other_key = list(fusion_hooks.keys())[-1]
                ego_key = list(ro_hooks.keys())[-1]
                other_tok = fusion_hooks[other_key][:, 1:128, :]  # (N,127,D)
                ego_tok = ro_hooks[ego_key][:, 0, :]  # (N,D)
...
def parse_args():
    ...
    p.add_argument("--csv-path", default="/data/full_version/intervention.csv")
    p.add_argument("--others-csv", default="/data/full_version/intervention_others.csv")
    p.add_argument(
        "--out-csv",
        default=None,
    ...
    p.add_argument("--model-path", default="/data/full_version/model/exp_80000_subset_aix")
    p.add_argument("--model-name", default="early_attn_s3_0908_113203.pth")
    ...
    p.add_argument(
        "--features-cache",
        default="/data/full_version/intervention_auto_label_frames.pkl",
...
def main():
    ...
    if need_sim_ids:
        ...
        ego_root = os.path.join(
            args.model_path, "ego_linear_prob", args.model_name.replace(".pth", "")
        )
        other_root = os.path.join(
            args.model_path, "other_linear_prob", args.model_name.replace(".pth", "")
        )
        ego_lps, other_lps = [], []
        for h in FUTURE_STEPS:
    ...
    if args.pick_by_metrics:
        from gpudrive.integrations.il.figure.intervention_label_eval import (
            eval_candidates_and_pick,
        )

        # Need policy + other LPs even if features were cached.
        model_path = os.path.join(args.model_path, args.model_name)
        policy = torch.load(model_path, weights_only=False).to("cuda")
        policy.eval()
        other_root = os.path.join(
        ...
        for sid in all_ids:
            ...
            other_root = os.path.join(
                args.model_path,
                "other_linear_prob",
                args.model_name.replace(".pth", ""),
            )
            other_lps = []
            for h in FUTURE_STEPS:
                op = os.path.join(
                    other_root, f"seed{args.seed}", f"pos_early_lp_{h}.pth"
                )
                if not os.path.exists(op):
                    op = os.path.join(other_root, f"seed{args.seed}", f"pos_lp_{h}.pth")
                ot = torch.load(op, weights_only=False).to("cuda")
