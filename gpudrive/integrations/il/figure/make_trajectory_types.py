...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", "-sd", type=str, default="/data/full_version/trajectory_types", help="directory to save outputs")
    ...
    TOTAL_NUM_WORLDS = 80000 if args.data_dir == "training" else 9987
    ...
    for idx in tqdm(range(num_iter)):
        ...
        # (scene_idx, veh_idx, label) 매핑
        scene_ego_keys = list(zip(index_array_np, ego_ids_np))
        id_to_label = {key: label for key, label in zip(scene_ego_keys, scene_labels)}

        # DataFrame 생성
        df = pd.DataFrame(
            [(s, v, id_to_label[(s, v)]) for (s, v) in scene_ego_keys],
            columns=["scene_idx", "veh_idx", "label"]
        )

        # CSV로 저장 (append 방식으로 전체 기록)
        os.makedirs(args.save_dir, exist_ok=True)
        csv_path = f"{args.save_dir}/{args.data_dir}_labels.csv"
        if not os.path.exists(csv_path):
            df.to_csv(csv_path, index=False, mode='w')
        else:
            df.to_csv(csv_path, index=False, mode='a', header=False)

        print(f"Saved {len(df)} labels for batch {idx}")
