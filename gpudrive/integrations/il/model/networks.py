'''
The attention part of original code is from https://github.com/vita-epfl/UniTraj
'''
    ...
    def forward(
        ...
        for q_chunk, k_chunk, v_chunk in zip(
            ...
            if self.separate_attn_weights:
                ...
                ego_attn = attn[..., 0, :].detach() # valid only if attn of road objects
                attn = self.dropout(attn)
...
class GMM(nn.Module):
    def __init__(self, network_type, input_dim, head_config, time_dim=1):
        ...
        self.cov_clip = head_config.cov_clip
    ...
    def get_gmm_params(self, x):
        ...
        covariances = torch.clamp(covariances, self.clip_value, self.cov_clip)
