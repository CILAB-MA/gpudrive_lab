            ...
            for var_name in self.full_var:
                if self.__dict__[var_name] is not None:
                    if var_name in ['obs', 'road_mask', 'partner_mask']:
                        ...
                        data = self.__dict__[var_name][idx1, idx2 + self.rollout_len + self.pred_len - 2 : idx2 + self.rollout_len + self.pred_len - 1] # idx 0 -> (0, 10 + 5 - 2) -> (0, 13) & padding = 9 -> end with last action timestep
                    elif var_name in ['aux_mask', 'other_pos', 'ego_pos', 'partner_labels']:
                        data = self.__dict__[var_name][idx1, idx2]
                    elif var_name in ["future_valid_mask"]:
                        data = self.__dict__[var_name][idx1, idx2:idx2 + 1]
