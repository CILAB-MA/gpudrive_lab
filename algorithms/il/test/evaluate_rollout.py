...
def parse_args():
    ...
    parser.add_argument('--rollout-timestep', '-rt', type=int, default=0)
...
if __name__ == "__main__":
    ...
    rollout_obs = [torch.zeros(obs).to(args.device) for _ in range(args.rollout_len - 1)]
    for time_step in range(env.episode_len):
        # for transformer
        rollout_obs.append(obs) # (T, num_world, 1, feature)
        if time_step % args.rollout_freq == 0: # timestep 10, 91 - 80, 
            action_idx = 0
            rollout_obs = torch.cat(rollout_obs[:-args.rollout_len]).transpose(0, 1) # (num_world, T, 1, input_feature)
            with torch.no_grad():
                rollout_actions = bc_policy(rollout_obs, deterministic=True) # (alive_agent, rollout_step, 3)
        all_actions = torch.zeros(obs.shape[0], obs.shape[1], 3).to(args.device)
        all_actions[~dead_agent_mask, :] = actions[~dead_agent_mask, action_idx] / args.action_scale #todo: check about alive_agent_num
