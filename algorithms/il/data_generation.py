...
def generate_state_action_pairs(
    ...
    expert_accel = raw_expert_action[0, 5, :, 0]
    expert_steer = raw_expert_action[0, 5, :, 1]
    ...
    speeds = [obs[0, 5, 0].unsqueeze(-1)]
    poss = [obs[0, 5, 3:5].unsqueeze(0)]
    ...
    for time_step in range(env.episode_len):
        ...
        # print(dones[0, 5])
        if dones[0, 5] == 0:
            speeds.append(next_obs[0, 5, 0].unsqueeze(-1))
            poss.append(next_obs[0, 5, 3:5].unsqueeze(0))
    ...
    axs[1, 0].plot(disc_expert_actions[0, 5, :, 0].numpy(), label='Simulation Accels', color='r')
    ...
    axs[1, 1].plot(disc_expert_actions[0, 5, :, 1].numpy(), label='Simulation Steers', color='r')
